...
struct OcpQpIpmSolverTpl<Scalar>::WrapperHolder {
  WrapperHolder()
    : ocp_qp_dim_wrapper(std::make_shared<hpipm::ocp_qp_dim_wrapper<Scalar>>()),
      ocp_qp_ipm_arg_wrapper(std::make_shared<hpipm::ocp_qp_ipm_arg_wrapper<Scalar>>()),
      ocp_qp_wrapper(),
      ocp_qp_sol_wrapper(),
      ocp_qp_ipm_ws_wrapper() {}
  ~WrapperHolder() = default;

  std::shared_ptr<hpipm::ocp_qp_dim_wrapper<Scalar>> ocp_qp_dim_wrapper;
  std::shared_ptr<hpipm::ocp_qp_ipm_arg_wrapper<Scalar>> ocp_qp_ipm_arg_wrapper;
  hpipm::ocp_qp_wrapper<Scalar> ocp_qp_wrapper;
  hpipm::ocp_qp_sol_wrapper<Scalar> ocp_qp_sol_wrapper;
  hpipm::ocp_qp_ipm_ws_wrapper<Scalar> ocp_qp_ipm_ws_wrapper;
};



template <typename Scalar>
OcpQpIpmSolverTpl<Scalar>::OcpQpIpmSolverTpl(const std::vector<OcpQp>& ocp_qp,
...
template <typename Scalar>
OcpQpIpmSolverTpl<Scalar>::OcpQpIpmSolverTpl(const OcpQpIpmSolverSettings& solver_settings)
  : solver_settings_(),
    solver_statistics_(),
    dim_(),
    wrapper_holder_(new WrapperHolder()) {
...
template <typename Scalar>
OcpQpIpmSolverTpl<Scalar>::~OcpQpIpmSolverTpl() {}


template <typename Scalar>
void OcpQpIpmSolverTpl<Scalar>::setSolverSettings(const OcpQpIpmSolverSettings& solver_settings) {
  using traits = hpipm_traits<Scalar>;
  auto* ocp_qp_ipm_arg_ptr = wrapper_holder_->ocp_qp_ipm_arg_wrapper->get();
  solver_settings_ = solver_settings;
  ...
  // Scalar copies of the double-valued settings so hpipm receives matching
  // precision pointers (s_*_set_* takes float*).
  Scalar mu0       = static_cast<Scalar>(solver_settings_.mu0);
  Scalar alpha_min = static_cast<Scalar>(solver_settings_.alpha_min);
  Scalar tol_stat  = static_cast<Scalar>(solver_settings_.tol_stat);
  Scalar tol_eq    = static_cast<Scalar>(solver_settings_.tol_eq);
  Scalar tol_ineq  = static_cast<Scalar>(solver_settings_.tol_ineq);
  Scalar tol_comp  = static_cast<Scalar>(solver_settings_.tol_comp);
  Scalar reg_prim  = static_cast<Scalar>(solver_settings_.reg_prim);
  traits::ocp_qp_ipm_arg_set_default(hpipm_mode, ocp_qp_ipm_arg_ptr);
  traits::ocp_qp_ipm_arg_set_mu0(&mu0, ocp_qp_ipm_arg_ptr);
  traits::ocp_qp_ipm_arg_set_iter_max(&solver_settings_.iter_max, ocp_qp_ipm_arg_ptr);
  traits::ocp_qp_ipm_arg_set_alpha_min(&alpha_min, ocp_qp_ipm_arg_ptr);
  traits::ocp_qp_ipm_arg_set_tol_stat(&tol_stat, ocp_qp_ipm_arg_ptr);
  traits::ocp_qp_ipm_arg_set_tol_eq(&tol_eq, ocp_qp_ipm_arg_ptr);
  traits::ocp_qp_ipm_arg_set_tol_ineq(&tol_ineq, ocp_qp_ipm_arg_ptr);
  traits::ocp_qp_ipm_arg_set_tol_comp(&tol_comp, ocp_qp_ipm_arg_ptr);
  traits::ocp_qp_ipm_arg_set_reg_prim(&reg_prim, ocp_qp_ipm_arg_ptr);
  traits::ocp_qp_ipm_arg_set_warm_start(&solver_settings_.warm_start, ocp_qp_ipm_arg_ptr);
  traits::ocp_qp_ipm_arg_set_pred_corr(&solver_settings_.pred_corr, ocp_qp_ipm_arg_ptr);
  traits::ocp_qp_ipm_arg_set_ric_alg(&solver_settings_.ric_alg, ocp_qp_ipm_arg_ptr);
  traits::ocp_qp_ipm_arg_set_split_step(&solver_settings_.split_step, ocp_qp_ipm_arg_ptr);
}


template <typename Scalar>
void OcpQpIpmSolverTpl<Scalar>::resize(const std::vector<OcpQp>& ocp_qp) {
  using traits = hpipm_traits<Scalar>;
  dim_.resize(ocp_qp);
  std::vector<int> ns(dim_.N+1);
  for (unsigned int i=0; i<=dim_.N; ++i) {
    ns[i] = dim_.nsbx[i] + dim_.nsbu[i] + dim_.nsg[i];
  }
  wrapper_holder_->ocp_qp_dim_wrapper->resize(dim_.N);
  traits::ocp_qp_dim_set_all(dim_.nx.data(), dim_.nu.data(),
                             dim_.nbx.data(), dim_.nbu.data(), dim_.ng.data(),
                             ns.data(),
                             wrapper_holder_->ocp_qp_dim_wrapper->get());
  // for initial state embedding
  traits::ocp_qp_dim_set_nx(0, 0, wrapper_holder_->ocp_qp_dim_wrapper->get());
  traits::ocp_qp_dim_set_nbx(0, 0, wrapper_holder_->ocp_qp_dim_wrapper->get());
  traits::ocp_qp_dim_set_ns(0, 0, wrapper_holder_->ocp_qp_dim_wrapper->get());
  ...
  setSolverSettings(solver_settings_);
...
template <typename Scalar>
HpipmStatus OcpQpIpmSolverTpl<Scalar>::solve(const VectorX& x0,
  ...
  auto* ocp_qp_ptr = wrapper_holder_->ocp_qp_wrapper.get();
  traits::ocp_qp_set_all(A_ptr_.data(), B_ptr_.data(), b_ptr_.data(),
                         ...
                         idxs_ptr_.data(), nullptr, lls_ptr_.data(), lus_ptr_.data(), ocp_qp_ptr);

  // masks
  for (int i=1; i<=dim_.N; ++i) {
    if (ocp_qp[i].lbx_mask.size() == dim_.nbx[i]) {
      lbx_mask_ptr_[i] = ocp_qp[i].lbx_mask.data();
      traits::ocp_qp_set_lbx_mask(i, lbx_mask_ptr_[i], ocp_qp_ptr);
    }
    if (ocp_qp[i].ubx_mask.size() == dim_.nbx[i]) {
      ubx_mask_ptr_[i] = ocp_qp[i].ubx_mask.data();
      traits::ocp_qp_set_ubx_mask(i, ubx_mask_ptr_[i], ocp_qp_ptr);
    }
  }
  for (int i=0; i<dim_.N; ++i) {
    if (ocp_qp[i].lbu_mask.size() == dim_.nbu[i]) {
      lbu_mask_ptr_[i] = ocp_qp[i].lbu_mask.data();
      traits::ocp_qp_set_lbu_mask(i, lbu_mask_ptr_[i], ocp_qp_ptr);
    }
    if (ocp_qp[i].ubu_mask.size() == dim_.nbu[i]) {
      ubu_mask_ptr_[i] = ocp_qp[i].ubu_mask.data();
      traits::ocp_qp_set_ubu_mask(i, ubu_mask_ptr_[i], ocp_qp_ptr);
    }
  }
  for (int i=0; i<=dim_.N; ++i) {
    if (ocp_qp[i].lg_mask.size() == dim_.ng[i]) {
      lg_mask_ptr_[i] = ocp_qp[i].lg_mask.data();
      traits::ocp_qp_set_lg_mask(i, lg_mask_ptr_[i], ocp_qp_ptr);
    }
    if (ocp_qp[i].ug_mask.size() == dim_.ng[i]) {
      ug_mask_ptr_[i] = ocp_qp[i].ug_mask.data();
      traits::ocp_qp_set_ug_mask(i, ug_mask_ptr_[i], ocp_qp_ptr);
    }
  }

  // solve QP
  auto* ocp_qp_sol_ptr = wrapper_holder_->ocp_qp_sol_wrapper.get();
  auto* ocp_qp_ipm_arg_ptr = wrapper_holder_->ocp_qp_ipm_arg_wrapper->get();
  auto* ocp_qp_ipm_ws_ptr = wrapper_holder_->ocp_qp_ipm_ws_wrapper.get();
  if (solver_settings_.warm_start) {
    for (int i=0; i<dim_.N; ++i) {
      traits::ocp_qp_sol_set_x(i+1, qp_sol[i+1].x.data(), ocp_qp_sol_ptr);
      traits::ocp_qp_sol_set_u(i, qp_sol[i].u.data(), ocp_qp_sol_ptr);
    }
  }
  traits::ocp_qp_ipm_solve(ocp_qp_ptr, ocp_qp_sol_ptr, ocp_qp_ipm_arg_ptr, ocp_qp_ipm_ws_ptr);

  // get solution
  qp_sol[0].x = x0;
  for (int i=0; i<dim_.N; ++i) {
    traits::ocp_qp_sol_get_x(i+1, ocp_qp_sol_ptr, qp_sol[i+1].x.data());
    traits::ocp_qp_sol_get_u(i, ocp_qp_sol_ptr, qp_sol[i].u.data());
    traits::ocp_qp_sol_get_pi(i, ocp_qp_sol_ptr, qp_sol[i+1].pi.data());
    traits::ocp_qp_ipm_get_ric_P(ocp_qp_ptr, ocp_qp_ipm_arg_ptr, ocp_qp_ipm_ws_ptr, i+1, qp_sol[i+1].P.data());
    traits::ocp_qp_ipm_get_ric_p(ocp_qp_ptr, ocp_qp_ipm_arg_ptr, ocp_qp_ipm_ws_ptr, i+1, qp_sol[i+1].p.data());
    traits::ocp_qp_ipm_get_ric_K(ocp_qp_ptr, ocp_qp_ipm_arg_ptr, ocp_qp_ipm_ws_ptr, i, qp_sol[i].K.data());
    traits::ocp_qp_ipm_get_ric_k(ocp_qp_ptr, ocp_qp_ipm_arg_ptr, ocp_qp_ipm_ws_ptr, i, qp_sol[i].k.data());
  }
  // Reconstruct Riccati-related terms and costate at stage 0
  // Feedback gain
  Lr0_.setZero();
  traits::ocp_qp_ipm_get_ric_Lr(ocp_qp_ptr, ocp_qp_ipm_arg_ptr, ocp_qp_ipm_ws_ptr, 0, Lr0_.data());
  Lr0_inv_.noalias() = (Lr0_.template triangularView<Eigen::Lower>()).solve(MatrixX::Identity(dim_.nu[0], dim_.nu[0]));
  ...
  Scalar res_stat, res_eq, res_ineq, res_comp;
  traits::ocp_qp_ipm_get_iter(ocp_qp_ipm_ws_ptr, &solver_statistics_.iter);
  traits::ocp_qp_ipm_get_max_res_stat(ocp_qp_ipm_ws_ptr, &res_stat);
  traits::ocp_qp_ipm_get_max_res_eq(ocp_qp_ipm_ws_ptr, &res_eq);
  traits::ocp_qp_ipm_get_max_res_ineq(ocp_qp_ipm_ws_ptr, &res_ineq);
  traits::ocp_qp_ipm_get_max_res_comp(ocp_qp_ipm_ws_ptr, &res_comp);
  solver_statistics_.max_res_stat = res_stat;
  solver_statistics_.max_res_eq   = res_eq;
  solver_statistics_.max_res_ineq = res_ineq;
  solver_statistics_.max_res_comp = res_comp;
  const int stat_m = 18;
  ...
  // get hpipm status
  int hpipm_status;
  traits::ocp_qp_ipm_get_status(ocp_qp_ipm_ws_ptr, &hpipm_status);
...
template <typename Scalar>
const OcpQpIpmSolverSettings& OcpQpIpmSolverTpl<Scalar>::getIpmSolverSettings() const {
  return solver_settings_;
}


template <typename Scalar>
const OcpQpIpmSolverStatistics& OcpQpIpmSolverTpl<Scalar>::getSolverStatistics() const {
  return solver_statistics_;
}


// Explicit instantiation for the supported precisions.
template class OcpQpIpmSolverTpl<double>;
template class OcpQpIpmSolverTpl<float>;
