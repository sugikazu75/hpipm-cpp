#ifndef HPIPM_CPP_OCP_QP_DIM_HXX_
#define HPIPM_CPP_OCP_QP_DIM_HXX_
...
OcpQpDim::OcpQpDim(const std::vector<OcpQpTpl<Scalar>>& ocp_qp) {
  resize(ocp_qp);
}


template <typename Scalar>
void OcpQpDim::resize(const std::vector<OcpQpTpl<Scalar>>& ocp_qp) {
  if (ocp_qp.empty()) {
...
template <typename Scalar>
void OcpQpDim::checkSize(const std::vector<OcpQpTpl<Scalar>>& ocp_qp) const {
  if (ocp_qp.size() != N+1) {
...
#endif // HPIPM_CPP_OCP_QP_DIM_HXX_
