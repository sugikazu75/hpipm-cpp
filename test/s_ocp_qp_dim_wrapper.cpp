...
// The dim wrapper holds only integer dimension arrays, so the float
// instantiation must behave identically to the double one. This mirrors
// d_ocp_qp_dim_wrapper.cpp using ocp_qp_dim_wrapper<float> to exercise the
// single-precision template path (traits dispatch to the s_ hpipm symbols).
using s_ocp_qp_dim_wrapper = ocp_qp_dim_wrapper<float>;

class s_ocp_qp_dim_wrapper_test : public ::testing::Test {
protected:
  virtual void SetUp() {
  }
...
bool hasNullptr(const s_ocp_qp_dim_wrapper& dim) {
...
bool hasOnlyNullptr(const s_ocp_qp_dim_wrapper& dim) {
  return dim.get()->nx == nullptr
      && dim.get()->nu == nullptr
      && dim.get()->nb == nullptr
      && dim.get()->nbx == nullptr
...
bool hasSameSize(const s_ocp_qp_dim_wrapper& dim1,
                 const s_ocp_qp_dim_wrapper& dim2) {
...
bool hasSamePtr(const s_ocp_qp_dim_wrapper& dim1,
                const s_ocp_qp_dim_wrapper& dim2) {
...
TEST_F(s_ocp_qp_dim_wrapper_test, move) {
  std::random_device rd;
  std::default_random_engine eng(rd());
  std::uniform_int_distribution<int> distr(0, 10);
  ...
  s_ocp_qp_dim_wrapper dim3(std::move(dim2));
  EXPECT_FALSE(hasNullptr(dim3));
  EXPECT_TRUE(hasSameSize(dim3, dim));
  EXPECT_FALSE(hasSamePtr(dim3, dim));
  EXPECT_THROW(EXPECT_TRUE(hasOnlyNullptr(dim2)), std::runtime_error);

  s_ocp_qp_dim_wrapper dim4(dim);
  s_ocp_qp_dim_wrapper dim5;
  dim5 = std::move(dim4);
...
TEST_F(s_ocp_qp_dim_wrapper_test, resize) {
  std::random_device rd;
  std::default_random_engine eng(rd());
  std::uniform_int_distribution<int> distr(0, 10);
